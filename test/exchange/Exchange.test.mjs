...
describe('Exchange', () => {
  beforeEach(async () => {
    ...
    initialSupply = (await quoteToken.totalSupply()).toNumber();
  ...
  describe('swapQuoteTokenForBaseToken', () => {
    ...
    it('Should handle unexpected increase in quote tokens', async () => {
      ...
      expect((await quoteToken.balanceOf(exchange.address)).toNumber()).to.equal(
        quoteTokenReserveQty.add(amountToAdd * 100).toNumber(),
      );
  ...
  describe('swapBaseTokenForQuoteToken', () => {
    ...
    it('Should revert when no liquidity is available', async () => {
      ...
      expect((await quoteToken.balanceOf(trader.address)).toNumber()).to.not.equal(0);
  ...
  describe('addLiquidity', () => {
    it('Should allow for adding quote token liquidity (only) after a rebase up has occurred, and correct withdraw of re-based qty', async () => {
      ...
      expect((await baseToken.balanceOf(liquidityProvider.address)).toNumber()).to.equal(
        liquidityProviderbaseTokenExpectedBalance,
      );
      expect((await quoteToken.balanceOf(liquidityProvider.address)).toNumber()).to.equal(
      ...
      expect((await baseToken.balanceOf(cleanAddress1)).toNumber()).to.equal(30000);
      expect((await quoteToken.balanceOf(cleanAddress1)).toNumber()).to.equal(150002);
      ...
      expect((await baseToken.balanceOf(liquidityProvider2.address)).toNumber()).to.equal(
        liquidityProvider2baseTokenExpectedBalance,
      );
      expect((await quoteToken.balanceOf(liquidityProvider2.address)).toNumber()).to.equal(
      ...
      expect((await baseToken.balanceOf(cleanAddress2)).toNumber()).to.equal(50000);
      expect((await quoteToken.balanceOf(cleanAddress2)).toNumber()).to.equal(250000);
    ...
    it('Should allow for adding quote and base token liquidity after a rebase up has occurred, and correct withdraw of re-based qty', async () => {
      ...
      // We want to add 200000 quote tokens to remove the decay
      // (alphaDecay / omega = 40000 / .2 = 200000)
    ...
    it('Should allow for adding base token liquidity (only) after a rebase down has occurred, and correct withdraw of re-based qty', async () => {
       ...
       *
       * LP1 gets back 9 A and 45 B tokens (90+90 = 180$)
       *  - experienced a 20% rebase down on half of his position
       * LP2 gets back 1 A and 5 B tokens (10+10 = 20$) - contributed post rebase.
       *
       * This difference is due to the rebase. LP1 experienced
       *  the initial rebase while LP2 contributed post rebase.
       *
       */
    ...
    it('Should allow for adding quote and base token liquidity after a rebase down has occurred, and correct withdraw of re-based qty', async () => {
      ...
      // confirm the 'decay' is equal to the re-based
      // amount times the previous iOmega (B/A). (this is betaDecay)
      const iOmega = quoteTokenLiquidityToAdd / baseTokenLiquidityToAdd;
      const quoteTokenDecay =
        ((await exchange.internalBalances()).baseTokenReserveQty -
          (await baseToken.balanceOf(exchange.address))) *
    ...
    it('Should handle adding new liquidity after removing liquidity when decay is present due to rebase up', async () => {
      ...
      // confirm the exchange now has the expected balance after rebase
      expect((await baseToken.balanceOf(exchange.address)).toNumber()).to.equal(1500000);
    ...
    it('Should handle adding new liquidity after removing liquidity when decay is present due to rebase down', async () => {
      ...
      // confirm the exchange now has the expected balance after rebase
      expect((await baseToken.balanceOf(exchange.address)).toNumber()).to.equal(500000);
    ...
    it('Should handle trivial amounts of base token decay properly (issue 1)', async () => {
      ...
      // required to resolve it. We should allow a user to resolve
      // it by providing the required amount
      ...
      expect((await exchange.balanceOf(liquidityProvider2.address)).toNumber()).to.not.equal(
        0,
      );
    });
    ...
    it('Should handle trivial amounts of base token decay properly (issue 3)', async () => {
      ...
      expect((await exchange.balanceOf(liquidityProvider2.address)).toNumber()).to.equal(0);
    });

    it('Should handle trivial amounts of quote token decay properly (issue 1)', async () => {
      // trivial amount of quote token decay with our quote
      // token being a fractional amount of our base
      // token... (ie omega < 1)
    ...
    it('Should revert if minimum quote token amount isnt satisfied when decay present', async () => {
      // create expiration 50 minutes from now.
      const expiration = Math.round(new Date().getTime() / 1000 + 60 * 50);
      const liquidityProvider = accounts[1];
      const liquidityProvider2 = accounts[2];
    ...
    it('Should revert if minimum quote token amount isnt satisfied when decay is not present', async () => {
      // create expiration 50 minutes from now.
      const expiration = Math.round(new Date().getTime() / 1000 + 60 * 50);
      const liquidityProvider = accounts[1];
      const liquidityProvider2 = accounts[2];
    ...
    it('Should revert if minimum base token amount isnt satisfied when decay is present', async () => {
      // create expiration 50 minutes from now.
      const expiration = Math.round(new Date().getTime() / 1000 + 60 * 50);
      const liquidityProvider = accounts[1];
      const liquidityProvider2 = accounts[2];
    ...
    it('Should revert if minimum base token amount isnt satisfied when decay is not present', async () => {
      // create expiration 50 minutes from now.
      const expiration = Math.round(new Date().getTime() / 1000 + 60 * 50);
      const liquidityProvider = accounts[1];
      const liquidityProvider2 = accounts[2];
  ...
  describe('removeLiquidity', () => {
    ...
    it('Should not allow a trade to drain all liquidity due to a rebase down', async () => {
      ...
      // the trader needs quote tokens to trade for base tokens,
      // in an attempt to drain all base tokens
      ...
      // the qty this would return quoted on the the internal
      // reserves (x and y) is more than the total balance in the exchange.
      ...
      // our internal math should prevent this from occurring by adjusting
      // the qty curve correctly and the below transaction should not revert.
      await exchange
        .connect(trader)
    ...
    it('Should return fees to correct liquidity provider', async () => {
      ...
      expect((await exchange.balanceOf(liquidityProvider1.address)).toNumber()).to.equal(
        amountToAdd,
      );
      ...
      // our second liquidity provider has to deal with the
      // current decay prior to adding additional liquidity.
      // we currently have base token decay (base tokens that
      // are not contributing to liquidity) to remedy this,
      ...
      expect((await baseToken.balanceOf(liquidityProvider1.address)).toNumber()).to.equal(0);
      expect((await quoteToken.balanceOf(liquidityProvider1.address)).toNumber()).to.equal(
        0,
      );
      ...
      expect((await exchange.balanceOf(liquidityProvider1.address)).toNumber()).to.equal(0);
      ...
      // we expect that Lp2 has the same 'value' of tokens and doesn't get any
      // fees that he wasn't a part of the pool during the trades occurring
      ...
      expect((await exchange.totalSupply()).toNumber()).to.equal(0);
    ...
    it('Should not return fees to liquidity provider who didnt experience any trades', async () => {
      // Note: this is different from the 'Should return fees to correct liquidity provider'
      // test due to the order of the withdrawal of LP tokens
      // by the liquidity provider.  Basically, we want to ensure that order doesn't matter.
      // LP1 can withdraw before or after LP2 and the accounting still
      // is correct.
      ...
      // to simplify the accounting we will send all base and
      // quote tokens from the withdrawal to a 'clean' address
      const cleanAddress = accounts[4].address;

      // confirm this address has no balances.
      expect((await baseToken.balanceOf(cleanAddress)).toNumber()).to.equal(0);
      expect((await quoteToken.balanceOf(cleanAddress)).toNumber()).to.equal(0);
    ...
    it('Should allow for user to supply liquidity, a rebase to occur, and correct withdraw of re-based qty', async () => {
      ...
      expect((await baseToken.balanceOf(liquidityProvider.address)).toNumber()).to.equal(0);
      expect((await quoteToken.balanceOf(liquidityProvider.address)).toNumber()).to.equal(0);
      expect((await exchange.balanceOf(liquidityProvider.address)).toNumber()).to.equal(
        amountToAdd,
      );
      ...
      // simulate a rebase by sending more tokens to our exchange contract.
      expect((await baseToken.balanceOf(exchange.address)).toNumber()).to.equal(amountToAdd);
      ...
      expect((await exchange.balanceOf(liquidityProvider.address)).toNumber()).to.equal(0);
    ...
    it('Should allow for user to supply liquidity and immediately withdrawal equal amounts', async () => {
      ...
      // check original balances
      expect((await baseToken.balanceOf(accounts[0].address)).toNumber()).to.equal(
        initialSupply,
      );
      expect((await quoteToken.balanceOf(accounts[0].address)).toNumber()).to.equal(
        initialSupply,
      );
      ...
      expect((await exchange.balanceOf(accounts[0].address)).toNumber()).to.equal(
        amountToAdd,
      );
    ...
    it('Should track fees correctly through a rebase down', async () => {
      ...
      // simulate a rebase down by sending tokens away from exchange 1/2 the
      // current amount base tokens.
